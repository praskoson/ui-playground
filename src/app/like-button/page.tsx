...
// https://www.frontend.fyi/v/framer-motion-like-animation
  ...
  return (
    <div className="grid h-[100dvh] w-full place-content-center">
      <div ref={scope}>
        <button
          onClick={onClick}
          className="relative rounded-full border-2 border-blue-600 px-6 py-1.5 text-2xl text-blue-600 transition-colors hover:bg-blue-100"
        >
          ...
          <span aria-hidden className="block h-8 overflow-hidden">
            {["M", "o", "t", "i", "o", "n"].map((letter, index) => (
              <span
                data-letter={letter}
                className="letter relative inline-block h-8 after:absolute after:left-0 after:top-full after:h-8 after:content-[attr(data-letter)]"
                key={`${letter}-${index}`}
              >
                {letter}
              </span>
